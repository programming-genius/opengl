...
int	TextureLoader::LoadTextureFromDisk(char* szFileName, glTexture* pglTexture)
{
	...
	else
	{
		...
		else if ((lstrcmpiA(szExtension, "jpg") == 0) ||
			(lstrcmpiA(szExtension, "jpeg") == 0))
		{
...
int	TextureLoader::LoadTextureFromRam(unsigned char* pData, int Length, glTexture* pglTexture, eglTexType TexType)
{
	...
	HANDLE hFile = CreateFileA(szTempFileName,
		GENERIC_WRITE,
		0,
		NULL,
		CREATE_ALWAYS,
...
int	TextureLoader::LoadTextureFromResource(unsigned int ResourceName, char* pResourceType, glTexture* pglTexture, eglTexType TexType)
{
		...
		(TexType == txGif))
	{
		...
		rc = LoadJPG_GIFResource(MAKEINTRESOURCEA(ResourceName), pResourceType, pglTexture);
	}
	...
	if (TexType == txTga)
	{
		...
		rc = LoadTGAResource(MAKEINTRESOURCEA(ResourceName), pResourceType, pglTexture);
	}
